  ...
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    let oldFunction = descriptor.value;
            descriptor.value = function () {
              ...
              if (!kcService)
               throw new Error("kcService not available."); 

             if (!kcService.isReady().getValue())
                throw new Error("kcService not initialized.");                
...
function getExceptionMessageFor(methodName: string, typeName: string, userId: string): string {
